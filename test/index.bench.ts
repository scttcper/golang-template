...
const RANGE_FIELD_TEMPLATE = '{{ range .items }}{{ .name }}, {{ end }}';
const RANGE_FIELD_VARS = {
...
describe('parse() — compile + render every call', () => {
  ...
  bench('range field', () => {
    parse(RANGE_FIELD_TEMPLATE, RANGE_FIELD_VARS);
  });
...
describe('compile() — render only (AST cached)', () => {
  ...
  const rangeFieldTmpl = compile(RANGE_FIELD_TEMPLATE);
  ...
  bench('range field', () => {
    rangeFieldTmpl.render(RANGE_FIELD_VARS);
  });
